    ...
    public static function obfuscateEmail($content)
    {
        ...
        if ($obfuscationMethod=="javascript") {
            ...
            foreach($matches[0] as $k=>$match)
            {
                ...
                $newtag = '<a href="' . $contacturl . '" onmouseover="this.href=xyz(' . $obfusc . ')' . ($emailextra ?  $emailextra : '') . ';" id ="obscuredadd' . $k . '"' . $matches[3][$k] . '>' . ( strpos($matches[4][$k], '@')===false ? $matches[4][$k] : $contacttitle ) . '</a>' ;
                if (strpos($matches[4][$k], '@')!==false) {
            ...
            foreach($matches[0] as $k=>$match)
            {
                ...
                $newtag = strpos($newtag, '@')===false ?  str_replace($matches[4][$k], self::string2unicode($matches[4][$k]), $newtag) : $newtag; // unicode link text if it contains an email address 
